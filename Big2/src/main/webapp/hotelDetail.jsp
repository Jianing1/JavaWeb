...
<body>
		...
		<div class="container">
			<div class="baseInfo">
				...
				<div class="detail">
					...
					<div class="score">
						<span>${hotel.score}</span><span>/5分</span>
					</div>
					<div class="msg">
						<span class="my-label"><b>酒店地址</b></span> <span>${hotel.address}</span>
					</div>
					<div class="msg">
						<span class="my-label"><b>酒店印象</b></span>
						<c:forEach items="${fn:split(hotel.msg, ',')}" var="msg">
							<span class="label label-warning">${msg}</span>
						</c:forEach>
					</div>
					<div class="msg">
						<span class="my-label"><b>官方电话</b></span> <span>${hotel.tel}</span>
					</div>
					<div class="price">
						<span class="my-label" style="color: gray;">房间</span> <span
							class="price-item price-item-active">${hotel.price}/${hotel.type}</span>
						<span class="price-item">${hotel.price2}/${hotel.type2}</span>
					</div>
					<div>
						<form action="hotel">
							<input type="hidden" name="method" value="addCar" /> <input
								type="hidden" name="cate" id="cate" value="1" />
							<div class="date">
								<span class="my-label">入住日期</span> <input type="date"
									name="start" />
							</div>
							<div class="date">
								<span class="my-label">退房日期</span> <input type="date" name="end" />
							</div>
	...
	<script type="text/javascript">
		...
		$('.price-item').click(function() {
			$(this).addClass('price-item-active').siblings().removeClass('price-item-active')
			$('#cate').val($(this).index())
		})
